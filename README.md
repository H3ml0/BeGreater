...
<style>
  ...
  /* Блоки разделов */
  .content-section, .video-section, .diamond-section, .equipment-section, .keep-fit {
    padding: 40px 20px;
  }

  /* Общий флекс для секций с изображениями и текстом */
  .section-content {
    display: flex;
    flex-wrap: wrap;
    gap: 20px;
...
<script>
  ...
  diamondItems.forEach(item => {
    item.addEventListener('mouseenter', () => {
      item.style.transform = 'rotate(45deg) scale(1.05)';
    });
    item.addEventListener('mouseleave', () => {
      item.style.transform = 'rotate(45deg) scale(1)';
    });
  });
</script>
</body>
